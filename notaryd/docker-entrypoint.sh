...
if [ "${NETWORK}" = "ledger-mainnet-1" ]; then
  ...
  dasel put -f /cosmos/config/app.toml -v "$MONAD_RPC_URL" evm.monad.rpc_url
  dasel put -f /cosmos/config/app.toml -v "0x8f" evm.monad.chain_id
  ...
  dasel put -f /cosmos/config/app.toml -v "https://rpc.starknet-testnet.lava.build:443" starknet.sepolia.rpc_url
